...
export const NetworkDiagnostic: React.FC<NetworkDiagnosticProps> = ({
  ...
  // 将诊断逻辑移到 useEffect 内部，避免 ESLint 警告
  useEffect(() => {
    if (!visible) {
      return;
    }
    ...
    const runDiagnostic = async () => {
      ...
      try {
        ...
        checks[0] = {
          ...
          message: hasVirtualAdapter ? '✓ 虚拟网卡已创建' : '✗ 虚拟网卡未找到',
      ...
      try {
        ...
        checks[1] = {
          ...
          message: firewallOk ? '✓ 防火墙规则正常' : '⚠ 防火墙可能阻止连接',
          solution: firewallOk
            ? undefined
      ...
      if (virtualIp) {
        ...
        try {
          ...
          checks[2] = {
            ...
            message: canPing ? '✓ 虚拟网络连通正常' : '✗ 无法 ping 通虚拟 IP',
            solution: canPing ? undefined : '虚拟网络可能未正确建立，请尝试重新创建大厅',
          };
        } catch {
          checks[2] = {
            name: '网络连通性检查',
      ...
      try {
        ...
        checks[checkIndex] = {
          ...
          message: udpOk ? '✓ UDP 端口 11010 可用' : '⚠ UDP 端口可能被占用',
          solution: udpOk ? undefined : '请关闭其他可能占用 UDP 11010 端口的程序',
        };
      } catch {
        const checkIndex = virtualIp ? 3 : 2;
        checks[checkIndex] = {
