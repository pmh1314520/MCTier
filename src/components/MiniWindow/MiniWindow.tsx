...
import { PlayerIcon, MicIcon, SpeakerIcon, CloseCircleIcon, CollapseIcon, CloseIcon, WarningTriangleIcon } from '../icons';
      ...
      {currentView === 'chat' ? (
        ...
        <div className="mini-window-header">
          <h3 className="mini-window-title">
            {collapsed && lobby ? (
              <>
                {lobby.name.length > 4 ? `${lobby.name.substring(0, 4)}...` : lobby.name} ({players.length + 1}人)
                {/* 收起状态下的新消息提示文本 */}
                {unreadCount > 0 && <span className="mini-new-message-text"> - 新消息</span>}
              </>
            ) : (
              'MCTier'
            )}
                ...
                >
                  <div className="lobby-card-header">
                    <h4 className="lobby-card-title">
                      {lobby.name.length > 8 ? `${lobby.name.substring(0, 8)}...` : lobby.name}
                    </h4>
                    <motion.button
