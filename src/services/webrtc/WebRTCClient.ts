...
export interface SignalingMessage {
  type: 'offer' | 'answer' | 'ice-candidate' | 'player-joined' | 'player-left' | 'status-update' | 'heartbeat';
  from?: string;
  to?: string;
  sdp?: string;
  candidate?: string;
...
export class WebRTCClient {
  ...
  // 事件回调
  private onPlayerJoinedCallback?: (playerId: string, playerName: string) => void;
      ...
      try {
        ...
        this.websocket.onopen = () => {
          ...
          if (this.websocket) {
            ...
            console.log('📤 已发送注册消息，玩家名称:', this.localPlayerName);
          }
    ...
    try {
      switch (message.type) {
        case 'players-list':
          ...
          for (const player of message.players) {
            ...
            if (this.onPlayerJoinedCallback) {
              this.onPlayerJoinedCallback(player.playerId, player.playerName);
            }
        ...
        case 'player-joined':
          ...
          // 触发回调
          if (this.onPlayerJoinedCallback) {
            this.onPlayerJoinedCallback(message.playerId, message.playerName);
          }
