...
- **💬 聊天室** - 大厅内实时文字聊天，支持消息历史记录
- **📁 文件夹共享** - P2P高速文件传输，支持密码保护、有效期设置、多线程下载，速度可达2-8MB/s
...
MCTier 提供了强大的 P2P 文件共享功能，让同大厅内的玩家可以高速传输文件。

#### 添加共享文件夹

1. 在大厅界面底部点击 **文件夹共享** 按钮
2. 点击 **添加共享文件夹** 按钮
3. 选择要共享的文件夹
5. 设置访问密码（可选，用于保护共享内容）
6. 设置有效期（可选，过期后自动移除共享）
7. 点击 **确定** 完成添加

**注意事项：**
- 建议共享的文件夹中单个文件不超过 100MB，避免多人同时下载时内存占用过高
- 共享文件夹会实时同步，添加或删除文件后其他玩家会立即看到
- 支持同时共享多个文件夹

#### 浏览和下载文件

1. 在文件夹共享界面，点击其他玩家共享的文件夹
2. 如果设置了密码，需要输入正确的密码才能访问
3. 浏览文件夹内容，支持批量下载文件夹中的所有文件
4. 支持多文件同时下载，每个文件最多使用 12 个线程并发下载
5. 在 **传输列表** 中查看下载进度和已完成的文件

**传输速度：**
- 局域网 P2P 直连：2-8 MB/s
- 采用 WebRTC DataChannel 技术，无需中转服务器
